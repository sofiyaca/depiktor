...
async function twitterApiFetch() {
  ...
  try {
    ...
    console.error(
      `Could not generate a Bearer token. Please check that your credentials
        are correct and that the Filtered Stream preview is enabled in your Labs dashboard. (${e})`
    );
    process.exit(-1);
  }
  ...
  for (let i = 0; i < technologies.length; i++) {
    const query = technologies[i].dataValues.name;
    requestConfig.qs.query = encodeURIComponent(query);

    let totalCount = 0;
    const body = await getCount(requestConfig);
    ...
    if (resultCount) {
      ...
      if (nextToken) {
        ...
        while (nextToken) {
          const next_body = await getCount(requestConfig);
    ...
    try {
      await technologies[i].createCount({ total: totalCount });
